...
def start(message):
    description = 'Я бот для создания списка дел. Жми кнопку или команду /add для добавдения'
...
def end_task(message):
    global user_state
    user_state = ''
    bot.reply_to(message, 'Выход из сеанса добавдения задачи ')
...
def keyboard(message):
    ...
    bot.send_message(message.chat.id, 'Text again', reply_markup=markup)
